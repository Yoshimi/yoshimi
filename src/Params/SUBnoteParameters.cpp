...
SUBnoteParameters::SUBnoteParameters() : Presets()
{
    ...
    bpfilterPool = new boost::pool<>(sizeof(bpfilter) * 5 * MAX_SUB_HARMONICS);
...
SUBnoteParameters::~SUBnoteParameters()
{
    ...
    bpfilterPool->purge_memory();
    buffPool->purge_memory();
}
...
void SUBnoteParameters::add2XML(XMLwrapper *xml)
{
    ...
    if (PFreqEnvelopeEnabled || !xml->minimal)
    {
    ...
    if (PBandWidthEnvelopeEnabled || !xml->minimal)
    {
    ...
    if (PGlobalFilterEnabled || !xml->minimal)
    {
...
void SUBnoteParameters::getfromXML(XMLwrapper *xml)
{
    ...
    if (xml->enterbranch("HARMONICS"))
    {
        ...
        for (int i = 0; i < MAX_SUB_HARMONICS; i++)
        {
            if (!xml->enterbranch("HARMONIC",i)) continue;
