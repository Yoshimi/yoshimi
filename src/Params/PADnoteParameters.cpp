...
float PADnoteParameters::getprofile(float *smp, int size)
{
    ...
    float modfreq = floorf(powf(2.0f,
                                 ...
                                 * 5.0f)) + 0.000001f);
    float modpar1 = powf((Php.modulator.par1 / 127.0f), 4.0f) * 5.0 / sqrtf(
        modfreq);
    float amppar1 =
    ...
    for(int i = 0; i < size * supersample; ++i) {
        ...
        switch(Php.amp.type) {
            ...
            case 2:
                amp = 0.5f
                      * (1.0f + cosf(PI * origx * sqrtf(amppar1 * 4.0f + 1.0f)));
                break;
            case 3:
                amp = 1.0f
                                                       ...
                                                       float bwadjust)
{
    ...
    for(int i = 0; i < synth->halfoscilsize; ++i)
        harmonics[i] /= max;
    for(int nh = 1; nh < synth->halfoscilsize; ++nh) { //for each harmonic
        ...
        if(ibw > profilesize) { // if the bandwidth is larger than the profilesize
            ...
            int   cfreq =
                lrintf(realfreq / (synth->halfsamplerate_f) * size) - ibw / 2;
            for(int i = 0; i < ibw; ++i) {
