...
#include "/home/louis/Documents/prog/Yoshimi/github/yoshimi/src/MidiControllerUI.h"
MidiCCWindow* midiccwindow = NULL; 

MidiCCRack::MidiCCRack(int x, int y, int w, int h) : Fl_Group(x,y,w,h) {
  embossedGroup = new Fl_Group(x, y, 300, 80);
...
void MidiCCRack::setMidiController(midiControl* contr) {
  controller = contr;
...
void MidiCCRack::startRecording(Fl_Widget* w, void* p) {
  MidiCCRack* caller = (MidiCCRack*)p;
       caller->controller->recording=1;
       //caller->controller->SpinnerInMidiCCPanel=(Fl_Spinner*)(caller->spinner);
}

void MidiCCRack::removeController(Fl_Widget* w, void* p) {
  MidiCCRack* caller = (MidiCCRack*)p;
       caller->synth->removeMidiControl(caller->controller);
       ((MidiCCWindow*)caller->parent()->parent())->updateRack();
}

void MidiCCRack::spinnerChanged(Fl_Widget* w, void* p) {
  MidiCCRack* caller = (MidiCCRack*)p;
          int value = caller->spinner->value();
          caller->controller->ccNbr = value;
}

void MidiCCRack::setMin(Fl_Widget* w, void* p) {
  MidiCCRack* caller = (MidiCCRack*)p;
       caller->controller->min = caller->dial->value();
}

void MidiCCRack::setMax(Fl_Widget* w, void* p) {
  MidiCCRack* caller = (MidiCCRack*)p;
       caller->controller->max = caller->dial->value();
}

MidiCCWindow::MidiCCWindow(int x, int y, int w, int h, SynthEngine* _synth, const char *title):Fl_Double_Window(300,300,w,h,title), synth(_synth) {
  scrollArea = new Fl_Scroll(0,0,this->w(),this->h(),"");
  end();
  updateRack();
}

void MidiCCWindow::Show() {
  updateRack();
  show();
}

void MidiCCWindow::updateRack() {
  scrollArea->clear();
