      ...
      portamento(portamento_)
{
    ...
    NoteGlobalPar.FilterCenterPitch =
        ...
        + pars->GlobalPar.PFilterVelocityScale / 127.0f * 6.0f
    ...
    if(pars->GlobalPar.PPunchStrength != 0) {
        ...
        NoteGlobalPar.Punch.initialvalue =
            ((powf(10.0f, 1.5f * pars->GlobalPar.PPunchStrength
                   / 127.0f) - 1.0f)
             * velF(velocity, pars->GlobalPar.PPunchVelocitySensing));
        float time =
            powf(10.0f, 3.0f * pars->GlobalPar.PPunchTime / 127.0f) / 10000.0f;          // 0.1 .. 100 ms
        float stretch = powf(440.0f / basefreq,
    ...
    for(int nvoice = 0; nvoice < NUM_VOICES; ++nvoice) {
        ...
        float vibratto_base_period  = 0.25f
            ...
            (1.0f
                                              ...
                                              / 127.0f) * 4.0f);
        for(int k = 0; k < unison; ++k) {
            unison_vibratto[nvoice].position[k] = synth->numRandom() * 1.8f
                                                  - 0.9f;
            float vibratto_period = vibratto_base_period * powf(
                2.0f,
                synth->
                numRandom() * 2.0f - 1.0f);                                                                                                                     //make period to vary randomly from 50% to 200% vibratto base period
        ...
        oscposhi_start += lrintf(
            (pars->VoicePar[nvoice].Poscilphase
             - 64.0f) / 128.0f * synth->oscilsize_f + synth->oscilsize_f * 4.0f);
        oscposhi_start %= synth->oscilsize;

        for(int k = 0; k < unison; ++k) {
            oscposhi[nvoice][k] = oscposhi_start;
            oscposhi_start      =
                (int)(synth->numRandom() * (synth->oscilsize - 1));              //put random starting point for other subvoices
        }
        ...
        switch(NoteVoicePar[nvoice].FMEnabled) {
            case PHASE_MOD:
                fmvoldamp = powf(440.0f / getVoiceBaseFreq(
                                     nvoice),
                                 pars->VoicePar[nvoice].PFMVolumeDamp / 64.0f);
        ...
        NoteVoicePar[nvoice].DelayTicks =
            lrintf((expf(pars->VoicePar[nvoice].PDelay / 127.0f
                         * logf(
                             50.0f))
                    - 1.0f) / synth->buffersize / 10.0f * synth->samplerate_f);
    }
...
void ADnote::ADlegatonote(float freq_,
                          float velocity_,
                          int portamento_,
                          int midinote_,
                          bool externcall)
    ...
    for(int nvoice = 0; nvoice < NUM_VOICES; ++nvoice) {
        ...
        switch(NoteVoicePar[nvoice].FMEnabled) {
            ...
            case FREQ_MOD:
                NoteVoicePar[nvoice].FMVolume =
                    (expf(pars->VoicePar[nvoice].PFMVolume / 127.0f
                          * FM_AMP_MULTIPLIER) - 1.0) * fmvoldamp * 4.0f;
                break;
            //    case PITCH_MOD:NoteVoicePar[nvoice].FMVolume=(pars->VoicePar[nvoice].PFMVolume/127.0*8.0)*fmvoldamp;//???????????
            //	          break;
            default:
    ...
    for(nvoice = 0; nvoice < NUM_VOICES; ++nvoice) {
        ...
        if(partparams->VoicePar[nvoice].PFreqEnvelopeEnabled)
            NoteVoicePar[nvoice].FreqEnvelope =
                new Envelope(partparams->VoicePar[nvoice].FreqEnvelope,
                             basefreq);
        ...
        if((NoteVoicePar[nvoice].FMEnabled != NONE)
           && (NoteVoicePar[nvoice].FMVoice < 0)) {
            ...
            int oscposhiFM_add =
                lrintf(
                    (partparams->VoicePar[nvoice].PFMoscilphase
                     - 64.0f) / 128.0f * synth->oscilsize_f
...
void ADnote::setfreq(int nvoice, float in_freq)
{
    for(int k = 0; k < unison_size[nvoice]; ++k) {
        ...
        oscfreqhi[nvoice][k] = (speed > 0.0f) ? lrintf(speed) : lrintf(
            speed - 1.0f);
...
void ADnote::setfreqFM(int nvoice, float in_freq)
{
    for(int k = 0; k < unison_size[nvoice]; ++k) {
        ...
        oscfreqhiFM[nvoice][k] = (speed > 0.0f) ? lrintf(speed) : lrintf(
            speed - 1.0f);
...
{
    float detune =
        NoteVoicePar[nvoice].Detune / 100.0f
        + NoteVoicePar[nvoice].FineDetune / 100.0f * ctl->bandwidth.relbw
        * bandwidthDetuneMultiplier + NoteGlobalPar.Detune / 100.0f;
...
void ADnote::computeCurrentParameters(void)
{
    ...
    globalpitch = 0.01f
                  * (NoteGlobalPar.FreqEnvelope->envout()
