...
void OscilGen::getbasefunction(float *smps)
{
    ...
    switch(Pbasefuncmodulation) {
        ...
        case 3:
            ...
            basefuncmodulationpar3 =
                0.01f
                + (powf(2.0f, basefuncmodulationpar3 * 16.0f) - 1.0f) / 10.0f;
            break;
    ...
    for(int i = 0; i < synth->oscilsize; ++i) {
        ...
        switch(Pbasefuncmodulation) {
            case 1:
                t = t * basefuncmodulationpar3
                    + sinf((t + basefuncmodulationpar2)
                           * 2.0f
                           * PI) * basefuncmodulationpar1;     // rev
                break;
            case 2:
                t = t
                    + sinf((t * basefuncmodulationpar3 + basefuncmodulationpar2)
                           * 2.0f * PI) * basefuncmodulationpar1; // sine
                break;
            case 3:
                t = t
                    + powf(((1.0f
                             - cosf((t + basefuncmodulationpar2) * 2.0f * PI))
                            * 0.5f),
...
void OscilGen::waveshape(void)
{
    ...
    // Do the waveshaping
    waveShapeSmps(synth->oscilsize,
                  tmpsmps.get(), Pwaveshapingfunction, Pwaveshaping);
...
void OscilGen::modulation(void)
{
    ...
    switch(Pmodulation) {
        case 1:
            modulationpar1 =
                (powf(2.0f, modulationpar1 * 7.0f) - 1.0f) / 100.0f;
        ...
        case 3:
            modulationpar1 =
                (powf(2.0f, modulationpar1 * 9.0f) - 1.0f) / 100.0f;
...
void OscilGen::spectrumadjust(void)
{
    ...
    float max = 0.0f;
    for(int i = 0; i < synth->halfoscilsize; ++i) {
        float tmp = powf(oscilFFTfreqs.c[i], 2.0f) + powf(oscilFFTfreqs.s[i],
                                                          2.0f);
        if(max < tmp)
            max = tmp;
    }
    ...
    for(int i = 0; i < synth->halfoscilsize; ++i) {
        float mag =
            sqrtf(powf(oscilFFTfreqs.s[i],
                       2) + powf(oscilFFTfreqs.c[i], 2.0f)) / max;
...
short int OscilGen::get(float *smps, float freqHz, int resonance)
{
    ...
    if(oldfilterpars != Pfiltertype * 256 + Pfilterpar1 + Pfilterpar2 * 65536
       + Pfilterbeforews * 16777216) {
        oscilprepared = 0;
        oldfilterpars =
            Pfiltertype * 256 + Pfilterpar1 + Pfilterpar2 * 65536
            + Pfilterbeforews * 16777216;
    }
    ...
    adaptiveharmonicpostprocess(&outoscilFFTfreqs.c[1],
                                synth->halfoscilsize - 1);
    adaptiveharmonicpostprocess(&outoscilFFTfreqs.s[1],
                                synth->halfoscilsize - 1);
...
//        srandom_r(randseed, &harmonic_random_buf);
        if(initstate_r(randseed, harmonic_random_state,
            ...
            Runtime.Log(
                "OscilGen failed to init harmonic amplitude amplitude randomness");
        ...
        switch(Pamprandtype) {
            ...
            case 2:
                ...
                for(int i = 1; i < nyquist - 1; ++i) {
                    float amp =
                        powf(fabsf(sinf(i * rndfreq)), power) * normalize;
                    outoscilFFTfreqs.c[i] *= amp;
                    outoscilFFTfreqs.s[i] *= amp;
                }
                break;
    ...
    for(int i = 1; i < n; ++i) {
        if(what == 0)
            ...
            if(Pcurrentbasefunc == 0)
                ...
                spc[i - 1] = sqrtf(
                    basefuncFFTfreqs.c[i] * basefuncFFTfreqs.c[i]
