...
#include "/home/louis/Documents/prog/Yoshimi/github/yoshimi/src/LV2_Plugin/WidgetPDialUI.h"
// WidgetPDial.cc
...
void WidgetPDial::callback(Fl_Callback* cb) {
  Fl_Widget::callback(cb);
}
  ...
  double dragsize, v, min = minimum(), max = maximum();
          ...
          switch (event)
          {
              ...
              case FL_ENTER:
                   //if(textset)
                   /*
                   Above was moved because it stopped scroll wheel
                   working when there was no tooltip.
                   */
                   {
                       if(!pos)
                       {
...
void WidgetPDial::reset(Fl_Widget* w, void* v) {
  Fl_Valuator* dial = (Fl_Valuator*)w;
          dial->value((dial->maximum()-dial->minimum())/2);
          dial->do_callback();
}

SynthEngine * WidgetPDial::getSynth() {
  return synth;
}

void WidgetPDial::setSynth(SynthEngine *_synth) {
  synth = _synth;
}

void WidgetPDial::stampa(Fl_Widget *w, void *v) {
  WidgetPDial *p = (WidgetPDial*)w;
