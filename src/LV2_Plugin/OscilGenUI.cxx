...
#include "/home/louis/Documents/prog/Yoshimi/github/yoshimi/src/LV2_Plugin/OscilGenUI.h"
// OscilgenUI.cc
...
void OscilSpectrum::init(OscilGen *oscil_,int oscbase_, SynthEngine *_synth) {
  synth = _synth;
  oscil = oscil_;
          oscbase = oscbase_;
}
...
void Oscilloscope::init(OscilGen *oscil_, SynthEngine *_synth) {
  synth = _synth;
  oscil=oscil_;
}

void Oscilloscope::init(OscilGen *oscil_,int oscbase_, SynthEngine *_synth) {
  synth = _synth;
  oscil=oscil_;
  oscbase=oscbase_;
}

void Oscilloscope::init(OscilGen *oscil_,int oscbase_,int phase_, SynthEngine *_synth) {
  synth = _synth;
  oscil=oscil_;
  oscbase=oscbase_;
  phase=phase_;
...
Fl_Group* Oscilharmonic::make_window() {
  { harmonic = new Fl_Group(0, 0, 90, 225);
    ...
    { PSlider* o = phase = new PSlider(0, 135, 15, 75);
      ...
      if (oscil->Phphase[n]==64) o->selection_color(0);
    } // PSlider* phase
...
void Oscilharmonic::init(OscilGen *oscil_,int n_,Fl_Group *display_,Fl_Widget *oldosc_,Fl_Widget *cbwidget_,Fl_Widget *applybutton_, SynthEngine *_synth) {
  synth = _synth;
  oscil=oscil_;
  n=n_;
  display=display_;
...
for (int i=0;i<MAX_AD_HARMONICS;i++){
  ...
  if (oscil->Phphase[i]==64) h[i]->phase->selection_color(0);
     else h[i]->phase->selection_color(222);
};
 ...
 each harmonic");
    ...
    { _this_has_to_be_the_last = new Fl_Scroll(5, 340, 660, 250);
      ...
      { Fl_Pack* o = harmonics = new Fl_Pack(10, 345, 650, 225);
        harmonics->type(1);
        for (int i=0;i<MAX_AD_HARMONICS;i++){h[i]=new Oscilharmonic(0,0,20,o->h(),"");h[i]->init(oscil,i,oscildisplaygroup,oldosc,cbwidget,applybutton, synth);}
        harmonics->end();
      } // Fl_Pack* harmonics
      _this_has_to_be_the_last->end();
    } // Fl_Scroll* _this_has_to_be_the_last
    string tname;
    if (oscil->ADvsPAD) tname = "PadSynth Harmonic Content"; else {tname = "AddSynth Oscillator "; tname += char(nvoice + 49);}
    tname = synth->getGuiMaster()->set_part_window_title(tname);
...
OscilEditor::OscilEditor(OscilGen *oscil_,Fl_Widget *oldosc_,Fl_Widget *cbwidget_,Fl_Widget *cbapplywidget_, int nvoice_, SynthEngine *_synth) {
  synth = _synth;
  oscil=oscil_;
  oldosc=oldosc_;
  cbwidget=cbwidget_;
