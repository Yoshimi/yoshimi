                  ...
                  int masterkeyshift)
{
    ...
    if(pos == -1)
        ...
        if(!Pkitmode) { // init the notes for the "normal mode"
            ...
            if(legatomodevalid) {
                ...
                if(kit[0].Ppadenabled)
                    partnote[posb].kititem[0].padnote =
                        new PADnote(kit[0].padpars, ctl, notebasefreq, vel,
                                    portamento, note, true);
                if(kit[0].Padenabled || kit[0].Psubenabled
                   || kit[0].Ppadenabled)
...
void Part::SetController(unsigned int type, int par)
{
    switch(type) {
        ...
        case C_soundcontroller8:
            ...
            for(int item = 0; item < NUM_KIT_ITEMS; ++item)
                if(kit[item].adpars)
                    kit[item].adpars->GlobalPar.Reson->sendcontroller(
                        C_soundcontroller8,
        ...
        case C_soundcontroller9:
            ...
            kit[0].adpars->GlobalPar.Reson->sendcontroller(
                C_soundcontroller9,
                ctl->
        ...
        case C_undefined102:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0)
               && (par < 2)) {
                kit[FilterLfoControlLsb].adpars->GlobalPar.GlobalFilter->
                Pcategory = par;
                kit[FilterLfoControlLsb].adpars->GlobalPar.GlobalFilter->
                changed = true;
            }
        ...
        case C_undefined103:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0)
               && (par < 4)) {
                kit[FilterLfoControlLsb].adpars->GlobalPar.GlobalFilter->Ptype
                    =
                        par;
                kit[FilterLfoControlLsb].adpars->GlobalPar.GlobalFilter->
                changed = true;
        ...
        case C_undefined105:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0) && (par < 128))
                kit[FilterLfoControlLsb].adpars->GlobalPar.FilterLfo->Pfreq =
                    (float)par / 127.0f;
            break;
        ...
        case C_undefined106:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0) && (par < 128))
                kit[FilterLfoControlLsb].adpars->GlobalPar.FilterLfo->
                Pintensity = par;
            break;
        ...
        case C_undefined108:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0) && (par < 128))
                kit[FilterLfoControlLsb].adpars->GlobalPar.FilterLfo->Pdelay =
                    par;
            break;
        ...
        case C_undefined109:
            if(kit[FilterLfoControlLsb].Padenabled && (par >= 0) && (par < 128))
                kit[FilterLfoControlLsb].adpars->GlobalPar.FilterEnvelope->
                PA_val = par;
            break;
        ...
        default:
            Runtime.Log(string("Ignoring midi control change type ")
                        + asString(type));
            break;
    }
}
...
void Part::setkeylimit(unsigned char keylimit)
{
    ...
    if(Ppolymode) {
        ...
        int oldestnotepos = -1;
        int maxtime = 0;
        if(notecount > limit)   // find out the oldest note
            for(int i = 0; i < POLIPHONY; ++i)
                if(((partnote[i].status == KEY_PLAYING)
                    || (partnote[i].status == KEY_RELASED_AND_SUSTAINED))
...
void Part::ComputePartSmps(void)
{
    ...
    for(k = 0; k < POLIPHONY; ++k) {
        ...
        for(int item = 0; item < partnote[k].itemsplaying; ++item) {
            int sendcurrenttofx =
                partnote[k].kititem[item].sendtoparteffect;
            ...
            if(adnote) {
                ...
                if(!quiet_please && adnote->ready) {
                    ...
                    for(int i = 0; i < synth->buffersize; ++i) { // add ADnote to part mix
                        partfxinputl[sendcurrenttofx][i] += tmpoutl[i];
                        partfxinputr[sendcurrenttofx][i] += tmpoutr[i];
                    }
                }
                if(adnote->finished()) {
                    Runtime.deadObjects->addBody(
                        partnote[k].kititem[item].adnote);
            ...
            if(subnote) {
                ...
                if(!quiet_please && subnote->ready) {
                    ...
                    for(int i = 0; i < synth->buffersize; ++i) { // add SUBnote to part mix
                        partfxinputl[sendcurrenttofx][i] += tmpoutl[i];
                        partfxinputr[sendcurrenttofx][i] += tmpoutr[i];
                    }
                }
                if(subnote->finished()) {
                    Runtime.deadObjects->addBody(
                        partnote[k].kititem[item].subnote);
            ...
            if(padnote) {
                ...
                if(!quiet_please && padnote->ready) {
                    ...
                    for(int i = 0; i < synth->buffersize; ++i) { // add PADnote to part mix
                        partfxinputl[sendcurrenttofx][i] += tmpoutl[i];
                        partfxinputr[sendcurrenttofx][i] += tmpoutr[i];
                    }
                }
...
bool Part::loadProgram(unsigned char bk, unsigned char prog)
{
    ...
    if(!xml.empty()) {
        ...
        if(xmlwrap->loadXML(xml))
            ...
            Runtime.Log("Failed to load xml data for program "
                        + asString(partBank)
...
bool Part::loadXMLinstrument(string filename)
{
    boost::shared_ptr<XMLwrapper> xmlwrap = boost::shared_ptr<XMLwrapper>(
        new XMLwrapper());
    if(!xmlwrap) {
        Runtime.Log("Import instrument failed to instantiate new XMLwrapper",
                    true);
        return false;
    }
    if(!xmlwrap->loadXMLfile(filename)) {
        Runtime.Log("Import instrument failed to xml->load file " + filename,
                    true);
        return false;
    }
    getfromXML(xmlwrap.get());
...
bool Part::importInstrument(string filename)
{
    ...
    getfromXML(xmlwrap.get());
    return progBanks->updateProgram(partBank,
                                    partProgram,
                                    Pname,
                                    xmlwrap->getXMLdata());
}
...
void Part::getfromXMLinstrument(XMLwrapper *xmlwrap)
{
    ...
    if(xmlwrap->enterbranch("INSTRUMENT_KIT")) {
        ...
        for(int i = 0; i < NUM_KIT_ITEMS; ++i) {
            ...
            kit[i].Psendtoparteffect = xmlwrap->getpar127(
                "send_to_instrument_effect",
                kit[i].
                Psendtoparteffect);
            kit[i].Padenabled = xmlwrap->getparbool("add_enabled",
                                                    kit[i].Padenabled);
            if(xmlwrap->enterbranch("ADD_SYNTH_PARAMETERS")) {
                kit[i].adpars->getfromXML(xmlwrap);
                xmlwrap->exitbranch();
            }
            kit[i].Psubenabled = xmlwrap->getparbool("sub_enabled",
                                                     kit[i].Psubenabled);
            if(xmlwrap->enterbranch("SUB_SYNTH_PARAMETERS")) {
                kit[i].subpars->getfromXML(xmlwrap);
                xmlwrap->exitbranch();
            }
            kit[i].Ppadenabled = xmlwrap->getparbool("pad_enabled",
                                                     kit[i].Ppadenabled);
    ...
    if(xmlwrap->enterbranch("INSTRUMENT_EFFECTS")) {
        for(int nefx = 0; nefx < NUM_PART_EFX; ++nefx) {
            ...
            Pefxroute[nefx] = xmlwrap->getpar("route",
                                              Pefxroute[nefx],
                                              0,
                                              NUM_PART_EFX);
            partefx[nefx]->setdryonly(Pefxroute[nefx] == 2);
