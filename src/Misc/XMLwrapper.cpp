      ...
      stackpos(0)
{
    ...
    tree = mxmlNewElement(MXML_NO_PARENT,
...
string XMLwrapper::getXMLdata(void)
{
    ...
    boost::shared_array<char> xml =
        boost::shared_array<char>(mxmlSaveAllocString(tree,
...
bool XMLwrapper::loadXMLfile(const string filename)
{
    ...
    if(!root) {
        Runtime.Log(
            "File " + filename + " doesn't contain valid data in this context");
        return false;
    }
    push(root);
    xml_version.major = string2int(mxmlElementGetAttr(root, "version-major"));
...
{
    node = mxmlFindElement(peek(), peek(), "par", "name",
...
{
    node = mxmlFindElement(peek(), peek(), "par_bool", "name",
                           name.c_str(), MXML_DESCEND_FIRST);
    if(!node)
        return defaultpar;
    const char *strval = mxmlElementGetAttr(node, "value");
...
{
    node = mxmlFindElement(peek(), peek(), "string", "name",
                           name.c_str(), MXML_DESCEND_FIRST);
    if(!node)
        return string();
    if(!node->child)
...
float XMLwrapper::getparreal(const string name,
                             float defaultpar,
                             float min,
                             float max)
{
...
mxml_node_t *XMLwrapper::addparams1(const string name,
                                    const string par1,
                                    const string val1)
{
    mxml_node_t *element = mxmlNewElement(node, name.c_str());
...
mxml_node_t *XMLwrapper::addparams2(const string name,
                                    const string par1,
                                    const string val1,
                                    const string par2,
                                    const string val2)
