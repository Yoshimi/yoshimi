        ...
        if(!alsaBad(snd_pcm_nonblock(audio.handle, 0), "set blocking failed"))
            if(prepHwparams())
                if(prepSwparams()) {
                    ...
                    if(!alsaBad(snd_pcm_info(audio.handle,
                                             pcminfo),
...
bool AlsaEngine::openMidi(WavRecord *recorder) // do openAudio() before openMidi()!
{
    ...
    if(snd_seq_open(&midi.handle, midi.device.c_str(), SND_SEQ_OPEN_INPUT,
                    0)) {
        Runtime.Log("Error, failed to open alsa midi device: " + midi.device);
        goto bail_out;
    }
    if(alsaBad(snd_seq_nonblock(midi.handle,
                                1), "Failed to set non-blocking on midi"))
        goto bail_out;

    // midi decoder
    if(alsaBad(snd_midi_event_new(MAX_MIDI_BYTES,
                                  &midi.decoder),
               "Failed to allocate alsa event decoder"))
        goto bail_out;
    ...
    snd_seq_client_info_event_filter_add(seqclientinfo,
    ...
    if(alsaBad(snd_seq_set_client_info(midi.handle,
                                       seqclientinfo),
               "Failed to set midi event filtering"))
        goto bail_out;
    ...
    if(alsaBad(snd_timer_query_open(&timerquery, "hw",
                                    0), "Alsa midi timers query failed"))
        goto bail_out;
    while(snd_timer_query_next_device(timerquery, timerid) >= 0) {
        ...
        timerstr = "hw:CLASS=" + asString(timerClass)
                   + ",SCLASS=" + asString(timerSClass) + ",CARD=" + asString(
            timerCard)
                   + ",DEV=" + asString(timerDevice) + ",SUBDEV=" + asString(
            timerSubDev);
        snd_timer_t *timerhandle;
        if(snd_timer_open(&timerhandle, timerstr.c_str(),
    ...
    if(alsaBad(snd_async_add_timer_handler(&midi.callbackHandler,
                                           midi.timerhandle,
    ...
    if(alsaBad(snd_timer_params(midi.timerhandle, timerparams),
        ...
        goto bail_out;
    Runtime.Log("Midi timer set to "
                + asString(snd_timer_params_get_ticks(timerparams)));

    midi.portId = snd_seq_create_simple_port(
        midi.handle,
        port_name,
...
bool AlsaEngine::Start(void)
{
    if((NULL != audio.handle)
       && !Runtime.startThread(&audio.pThread, _audioThread, this, true, false))
        goto bail_out;
    if(midi.timerhandle
       && alsaBad(snd_timer_start(midi.timerhandle),
                  "Failed to start midi timer")) {
...
bool AlsaEngine::prepHwparams(void)
{
    ...
    if(!alsaBad(snd_pcm_hw_params_set_buffer_time_near(audio.handle, hwparams,
        ...
        if(alsaBad(snd_pcm_hw_params_get_buffer_size(hwparams,
            ...
            goto bail_out;
        if(alsaBad(snd_pcm_hw_params_set_period_time_near(audio.handle,
                                                          hwparams,
                                                          &audio.period_time,
                                                          NULL),
                   "failed to set period time"))
            goto bail_out;
        if(alsaBad(snd_pcm_hw_params_get_period_size(hwparams,
                                                     &audio.period_size, NULL),
        ...
        if(alsaBad(snd_pcm_hw_params_set_buffer_size_near(audio.handle,
                                                          hwparams,
                                                          &audio.buffer_size),
                   "failed to set buffer size"))
            goto bail_out;
    }
    if(alsaBad(snd_pcm_hw_params(audio.handle,
                                 hwparams),
    ...
    if(ask_buffersize != audio.period_size)
        Runtime.Log("Asked for buffersize " + asString(ask_buffersize)
                    + ", Alsa dictates "
                    + asString((unsigned int)audio.period_size));
    return true;
    ...
    while(Runtime.runSynth) {
        ...
        if(audio.pcm_state != SND_PCM_STATE_RUNNING)
            switch(audio.pcm_state) {
                ...
                default:
                    Runtime.Log(
                        "AlsaEngine::audioThread, weird SND_PCM_STATE: "
...
bool AlsaEngine::alsaBad(int op_result, string err_msg)
{
    ...
    if(isbad)
        Runtime.Log("Error, alsa audio: " + err_msg + ": "
                    + string(snd_strerror(op_result)), true);
    return isbad;
}
