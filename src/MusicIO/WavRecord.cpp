...
bool WavRecord::SetFile(string fpath, string &errmsg)
{
    ...
    if(isRegFile(wavFile)) {
        ...
        if((wavOutInfo.samplerate != (int)samplerate)
...
bool WavRecord::SetOverwrite(string &errmsg)
{
    ...
    errmsg = "Error opening new wav file " + wavFile + " : " + string(
        sf_strerror(NULL));
    return false;
}
...
void WavRecord::Feed(float *samples_left, float *samples_right)
{
    ...
    for(unsigned int frame = 0; frame < bufferframes; ++frame) { // interleave floats
    ...
    if(wrote != bufferframes)
        Runtime.Log("Short write in feedRecord, "
                    + asString((unsigned int)wrote) + " / "
                    + asString(bufferframes));
}
...
{
    boost::shared_array<float> tferBuf =
        boost::shared_array<float>(new float[tferSamples]);
    interleavedFloats =
        boost::shared_array<float>(new float[sizeof(float) * buffersize * 2]);
    ...
    if(!toFifo) {
        Runtime.Log("Error, failed to open toFifo: " + string(strerror(
                                                                  errno)), true);
        pthread_exit(NULL);
    }
