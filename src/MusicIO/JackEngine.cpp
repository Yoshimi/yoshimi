...
bool JackEngine::openJackClient(string server)
{
    ...
    if(jackClient) {
        ...
        Runtime.Log("Failed jack_client_open(), status: "
                    + Runtime.asHexString((int)jstatus), true);
    return false;
}
...
{
    audioPortL = jack_port_register(jackClient,
                                    "left",
                                    JACK_DEFAULT_AUDIO_TYPE,
                                    JackPortIsOutput,
                                    0);
    audioPortR = jack_port_register(jackClient,
                                    "right",
                                    JACK_DEFAULT_AUDIO_TYPE,
                                    JackPortIsOutput,
                                    0);
...
bool JackEngine::openMidi(WavRecord *recorder)
{
    ...
    midiPort = jack_port_register(jackClient,
                                  port_name,
    ...
    if(NULL != jackClient) {
        ...
        if(audioPortL) {
            if((chk = jack_port_unregister(jackClient, audioPortL)))
                Runtime.Log(
                    "Failed to unregister audioPortL, status: " + asString(chk));
            audioPortL = NULL;
        }
        if(audioPortR) {
            if((chk = jack_port_unregister(jackClient, audioPortR)))
                Runtime.Log(
                    "Failed to unregister audioPortR, status: " + asString(chk));
            audioPortR = NULL;
        }
        if(NULL != midiPort) {
...
int JackEngine::processCallback(jack_nframes_t nframes)
{
    ...
    if(audioactive) {
        __sync_bool_compare_and_swap(&periodstartframe,
                                     periodstartframe,
                                     jack_last_frame_time(jackClient));
        __sync_bool_compare_and_swap(&periodendframe,
                                     periodendframe,
                                     periodstartframe + jackNframes);
    }
        ...
        for(unsigned int idx = 0; ok && idx < eventCount; ++idx) {
            ...
            switch(jEvent.buffer[0] & 0xf0) {
                ...
                case MSG_pitchwheel_control:
                    msg.event_frame = jEvent.time + __sync_or_and_fetch(
                        &periodstartframe,
                        0);
...
{
    lastevent = event;
    Runtime.setJackSessionSave(event->type,
                               event->session_dir,
                               event->client_uuid);
}
